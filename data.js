//---------------------------------------------------------

// Questions
/*
data = [
    ["why", "warum", ""],
    ["when", "wann", ""],
    ["who", "wer", ""],
    ["how", "wie", ""],
    ["what", "was", ""],
    ["which", "welche", ""],
    ["where", "wo", "position, location"],
    ["where from", "woher", "coming from a place"],
    ["where to", "wohin", "direction, moving somwhere"],
    ["how much", "wie viel", ""],
    ["how many", "wie viele", ""],
    ["How are you?", "Wie geht's?", "capital letter at start"],
    ["Which languages do you speak?", "Welche Sprachen sprechen Sie?", "capital letter at start"],
    ["What's your name?", "Wie heissen Sie?", "informal - shorten the word is to 's and capital letter at start"]
];
//*/

//---------------------------------------------------------


// simple sentences or phrases
/*
data = [
    ["Thanks", "Danke", ""],
    ["Hello", "Hallo", ""],
    ["Ja", "Ja", ""],
    ["No", "Nein", ""],
    ["Please", "Bitte", ""],
    ["Good day", "Guten Tag", ""],
    ["Good morning", "Guten Morgen", ""],
    ["Good evening", "Guten Abend", ""],
    ["Good night", "Gute Nacht", ""],
    ["Goodbye", "Auf Wiedersehen", ""],
    ["Bye", "Tschüss", ""],
    ["See you soon", "Bis bald", ""],
    ["See you tomorrow", "Bis morgen", ""],
    ["Welcome", "Willkommen", ""],
    ["You're welcome", "Gern geschehen", ""],
    ["I am sorry", "Es tut mir leid", ""],
    ["How is it going?", "Wie geht's?", ""],
    ["I am doing well", "Mir geht's gut", ""],
    ["No idea", "Keine Ahnung", ""],
    ["All right", "Alles klar", ""],
    ["Germany", "Deutschland", ""],
    ["My name is", "Ich heisse", ""],
    ["unfortunately", "Leider", ""],
    ["Exactly", "Genau", ""],
    ["Sorry", "Entschuldigung", ""]
];
//*/

//---------------------------------------------------------

// nouns
/*
data = [
    ["a", "eine", "femenine"],
    ["a", "ein", "masculine"],
    ["a", "ein", "neuter"],
    ["the", "der", "masculine"],
    ["the", "die", "femenine"],
    ["the", "das", "neuter"],
    ["the", "die", "plural"],
    ["the man", "der Mann", "masculine"],
    ["the men", "die Männer", "masculine"],
    ["the woman", "die Frau", "femenine"],
    ["the women", "die Frauen", ""],
    ["the boy", "der Junge", "masculine"],
    ["the boys", "die Jungen", "masculine"],
    ["the girl", "das Mädchen", ""],
    ["the girls", "die Mädchen", ""],
    ["the child", "das Kind", "neutral"],
    ["the children", "die Kinder", "neutral"],
    ["the water", "das Wasser", ""],
    ["the bread", "das Brot", ""],
    ["the name", "der Name", ""],
    ["the names", "die Namen", ""],
    ["the first name", "der Vorname", ""],
    ["the first names", "die Vornamen", ""],
    ["the last name", "der Nachname", ""],
    ["the last names", "die Nachnamen", ""],
    ["the telephone number", "die Telefonnummer", ""],
    ["the telephone numbers", "die Telefonnummern", ""],
    ["the mobile phone number", "die Handynummer", ""],
    ["the mobile phone numbers", "die Handynummern", ""],
    ["the street number", "die Hausnummer", ""],
    ["the street numbers", "die Hausnummern", ""],
    ["the city", "die Stadt", ""],
    ["the cities", "die Städte", ""],
    ["the country", "das Land", ""],
    ["the countries", "die Länder", ""],
    ["the book", "das Buch", ""],
    ["the books", "die Bücher", ""],
    ["the photo", "das Foto", ""],
    ["the photos", "die Fotos", ""],
    ["the free time", "die Freizeit", ""],
    ["the hobby", "das Hobby", ""],
    ["the student", "der Student", "masculine"],
    ["the students", "die Studenten", "masculine"],
    ["the student", "die Studentin", "femenine"],
    ["the students", "die Studentinnen", "femenine"],
    ["the stool", "der Stuhl", "masculine"],
    ["the stools", "die Stühle", "masculine"],
    ["the bus", "der Bus", ""],
    ["the busses", "die Busse", ""],
    ["the bike", "das Fahrrad", ""],
    ["the bikes", "die Fahrräder", ""],
    ["the plane", "das Flugzeug", ""],
    ["the planes", "die Flugzeuge", ""],
    ["the airport", "der Flughafen", ""],
    ["the airports", "die Flughäfen", ""],
    ["the train station", "der Bahnhof", ""],
    ["the train stations", "die Bahnhöfe", ""],
    ["the river", "der Fluss", ""],
    ["the rivers", "die Flüsse", ""],
    ["the harbor", "der Hafen", ""],
    ["the harbors", "die Häfen", ""],
    ["the hotel", "das Hotel", ""],
    ["the hotels", "die Hotels", ""],
    ["the church", "die Kirche", ""],
    ["the churches", "die Kirchen", ""],
    ["the market", "der Markt", ""],
    ["the markets", "die Märkte", ""],
    ["the town hall", "das Rathaus", ""],
    ["the town halls", "die Rathäuser", ""]
];
//*/


//---------------------------------------------------------

// calender
/*
data = [
    ["Monday", "Montag", ""],
    ["Tuesday", "Dienstag", ""],
    ["Wednesday", "Mittwoch", ""],
    ["Thursday", "Donnerstag", ""],
    ["Friday", "Freitag", ""],
    ["Saturday", "Samtag", ""],
    ["Sunday", "Sonntag", ""],
    ["January", "Januar", ""],
    ["February", "Februar", ""],
    ["March", "März", ""],
    ["April", "April", ""],
    ["May", "Mai", ""],
    ["June", "Juni", ""],
    ["July", "Juli", ""],
    ["August", "August", ""],
    ["September", "September", ""],
    ["October", "Oktober", ""],
    ["November", "November", ""],
    ["December", "Dezember", ""],
    ["Second", "Sekunde", ""],
    ["Minute", "Minute", ""],
    ["Hour", "Stunde", ""],
    ["Day", "Tag", ""],
    ["Week", "Woche", ""],
    ["Weekend", "Wochenende", ""],
    ["Month", "Monat", ""],
    ["Year", "Jahr", ""]
];
//*/


//---------------------------------------------------------

// verbs
/*
data = [
    // ["I am",     "ich bin", ""],
    // ["you are",  "du bist", "singular"],
    // ["he is",    "er ist", ""],
    // ["she is",   "sie ist", ""],
    // ["it is",    "es ist", ""],
    // ["we are",   "wir sind", ""],
    // ["you are",  "ihr seid", "plural"],
    // ["they are", "sie sind", ""],
    // ["you are",  "Sie sind", "formal (singular och plural)"],
    // ["I have",     "ich habe", ""],
    // ["you have",  "du hast", "singular"],
    // ["he has",    "er hat", ""],
    // ["she has",   "sie hat", ""],
    // ["it has",    "es hat", ""],
    // ["we have",   "wir haben", ""],
    // ["you have",  "ihr habt", "plural"],
    // ["they have", "sie haben", ""],
    // ["you have",  "Sie haben", "formal (singular and plural"],
    // ["I drink", "ich trinke", ""],
    // ["you drink", "du trinkst", "singular"],
    // ["he drinks", "er trinkt", ""],
    // ["she drinks", "sie trinkt", ""],
    // ["it drinks", "es trinkt", ""],
    // ["we drink", "wir trinken", ""],
    // ["you drink", "ihr trinkt", "plural"],
    // ["they drink", "sie trinken", ""],
    // ["you drink", "Sie trinken", "formal"],

    // ["I", "-e", "verb ending"],
    // ["you", "-st", "verb ending - singular"],
    // ["he/she/it", "-t", "verb ending"],
    // ["we", "-en", "verb ending"],
    // ["you", "-t", "verb ending - informal plural"],
    // ["they", "-en", "verb ending"],
    // ["you", "-en", "verb ending - formal singular and plural"],

    ["be called", "heissen", "verb base form"],
    ["come", "kommen", "verb base form"],
    ["learn", "lernen", "verb base form"],
    ["be", "sein", "verb base form"],
    ["speak", "sprechen", "verb base form"],
    ["live", "wohnen", "verb base form"],
    ["chat", "chatten", "verb base form"],
    ["cook", "kochen", "verb base form"],
    ["read", "lesen", "verb base form"],
    ["travel", "reisen", "verb base form"],
    ["swim", "schwimmen", "verb base form"],
    ["sing", "singen", "verb base form"],
    ["play", "spielen", "verb base form"],
    ["dance", "tanzen", "verb base form"],
    ["hear", "hören", "verb base form"],
    ["drive", "fahren", "verb base form"],
    ["buy", "kaufen", "verb base form"],
    ["look at", "ansehen", "verb base form"],
    ["visit", "besuchen", "verb base form"],
    ["eat", "essen", "verb base form"],
    ["go", "gehen", "verb base form"],
    ["see", "sehen", "verb base form"]
];
//*/

//---------------------------------------------------------


// numbers
/*
data = [
    ["0", "null", ""],
    ["1", "eins", ""],
    ["2", "zwei", ""],
    ["3", "drei", ""],
    ["4", "vier", ""],
    ["5", "fünf", ""],
    ["6", "sechs", ""],
    ["7", "sieben", ""],
    ["8", "acht", ""],
    ["9", "neun", ""],
    ["10", "zehn", ""],
    ["11", "elf", ""],
    ["12", "zwölf", ""],
    ["13", "dreizehn", ""],
    ["14", "vierzehn", ""],
    ["15", "fünfzehn", ""],
    ["16", "sechzehn", ""],
    ["17", "siebzehn", ""],
    ["18", "achtzehn", ""],
    ["19", "neunzehn", ""],
    ["20", "zwanzig", ""],
    ["21", "einundzwanzig", ""],
    ["22", "zweiundzwanzig", ""],
    ["23", "dreiundzwanzig", ""],
    ["24", "vierundzwanzig", ""],
    ["25", "fünfundzwanzig", ""],
    ["26", "sechsundzwanzig", ""],
    ["27", "siebenundzwanzig", ""],
    ["28", "achtundzwanzig", ""],
    ["29", "neunundzwanzig", ""],
    ["30", "dreissig", ""],
    ["31", "einunddressig", ""],
    ["36", "sechsunddressig", ""],
    ["39", "neununddressig", ""],
    ["40", "vierzig", ""],
    ["42", "zweiundvierzig", ""],
    ["50", "fünfzig", ""],
    ["55", "fünfundfünfzig", ""],
    ["60", "sechzig", ""],
    ["64", "vierundsechzig", ""],
    ["70", "siebzig", ""],
    ["78", "achtundsiebzig", ""],
    ["80", "achtzig", ""],
    ["82", "zweiundachtzig", ""],
    ["90", "neunzig", ""],
    ["99", "neunundneunzig", ""],
    ["100", "hundert", ""] 
];
//*/

//---------------------------------------------------------


// Wood, market, 
/*
data = [
// ["the apple", "der Apfel", ""],
// ["the apples", "die Äpfel", ""],
// ["the banana", "die Banane", ""],
// ["the bananas", "die Bananen", ""],
// ["the pear", "die Birne", ""],
// ["the pears", "die Birnen", ""],
// ["the kiwi", "die Kiwi", ""],
// ["the kiwis", "die Kiwis", ""],
// ["the orange", "die Orange", ""],
// ["the oranges", "die Orangen", ""]
["foods", "Lebensmittel", ""],
["the fruit", "das Obst", ""],
["the vegetable", "das Gemüse", ""],
["bread and bakery", "Brot und Gebäck", ""],
["meat", "Fleisch", ""],
["fish", "Fisch", ""],
["milk products", "Milchprodukte", ""],
["other foods", "andere Lebensmittel", ""],
["the business", "das Geschäft", ""],
["packaging", "Verpackungen", ""],
["the drink", "das Getränk", ""],
["the food", "das Essen", ""],
["the breakfast", "das Frühstuck", ""],
["the lunch", "das Mittagessen", ""],
["the dinner", "das Abendessen", ""],
["the cuecumber", "", ""],
["", "", ""],
["", "", ""],
["", "", ""],
["", "", ""],
["", "", ""],
["", "", ""],
["", "", ""] 
];
//*/

//---------------------------------------------------------

// verb
/*
data = [
    ["would like to/want to", "möchten", ""],
    ["must", "müssen", ""],
    ["can", "können", ""],
    ["want", "wollen", ""],
    ["need", "brauchen", ""],
    ["have", "haben", ""],
    ["make", "machen", ""],
    ["cook", "kochen", ""],
    ["eat", "essen", ""],
    ["buy", "kaufen", ""],
    ["like something/someone", "mögen", ""],
    ["drive", "fahren", ""],
    ["give", "geben", ""],
    ["read", "lesen", ""],
    ["speak", "sprechen", ""],
    ["go", "gehen", ""],
    ["meet", "treffen", ""] 
    ["see", "sehen", ""] 
    ];
    //*/
    
    //---------------------------------------------------------


// random words for last test
// third element gives guide to plural
//*
data = [
    ["apple", "der Apfel", "¨"],
    ["banana", "die Banane", "-n"],
    ["pear", "die Birne", "-n"],
    ["kiwi", "die Kiwi", "-s"],
    ["orange", "die Orange", "-n"],
    ["cucumber", "die Gurke", "-n"],
    ["potato", "die Kartoffel", "-n"],
    ["salad", "der Salat", "-e"],
    ["tomato", "die Tomate", "-n"],
    ["onion", "die Zwiebel", "-n"],
    ["bread", "das Brot", "-e"],
    ["cookie", "der Keks", "-e"],
    ["cake", "der Kuchen", "-"],
    ["egg", "das Ei", "-er"],
    ["fish", "der Fisch", "-e"],
    ["meat", "das Fleisch", ""],
    ["chicken", "das Hähnchen", "-"],
    ["ham", "der Schinken", "-"],
    ["butter", "die Butter", "-"],
    ["joghurt", "der Joghurt", "-s"],
    ["chesse", "der Käse", "-"],
    ["suger", "der Zucker", "-"],
    ["salt", "das Salz", "-"],
    ["oil", "das Öl", "-e"],
    ["noodle", "die Nudel", "-n"],
    ["rice", "der Reis", "-"],
    ["fruit", "das Obst", "-"],
    ["vegetable", "das Gemüse", "-"],
    ["bun", "das Brötchen", "-"],
    ["supermarket", "der Supermarkt", "-märkte"],
    ["cup", "der Becher", "-"],
    ["flask", "die Flasche", "-n"],
    ["glas", "das Glas", "-äser"],
    ["package", "die Packung", "-en"],
    ["Bag", "die Tüte", "-n"],
    ["kilogram", "das Kilogramm", ""],
    ["gram", "das Gramm", ""],
    ["liter", "der Liter", ""],
    ["beer", "das Bier", "-e"],
    ["water", "das Wasser", "-"],
    ["cola", "die Cola", "-s"],
    ["milk", "die Milch", ""],
    ["coffe", "der Kaffee", "-s"],
    ["juice", "der Saft", "-äfte"],
    ["wine", "der Wein", "-e"],
    ["tea", "der Tee", "-s"],
    ["food", "das Essen", "-"],
    ["breakfast", "das Frühstück", "-"],
    ["lunch", "das Mittagessen", "-"],
    ["dinner", "das Abendessen", "-"],
    ["baby", "das Baby", "-s"],
    ["wife", "die Ehefrau", "-en"],
    ["husband", "der Ehemann", "-männer"],
    ["parents", "die Eltern", "(plural)"],
    ["family", "die Familie", "-n"],
    ["mother", "die Mutter", "Mütter"],
    ["father", "der Vater", "Väter"],
    ["son", "der Sohn", "Söhne"],
    ["daughter", "die Tochter", "Töchter"],
    ["brother", "der Bruder", "Brüder"],
    ["sister", "die Schwester", "-n"],
    ["the city", "die Stadt", "-ädter"],
    ["the country", "das Land", "-änder"],
    ["the book", "das Buch", "-ücher"],
    ["the photo", "das Foto", "-s"],
    ["the free time", "die Freizeit", "-"],
    ["the hobby", "das Hobby", "-s"],
    ["the stool", "der Stuhl", "-ühle"],
    ["the bus", "der Bus", "-se"],
    ["the bike", "das Fahrrad", "-räder"],
    ["the plane", "das Flugzeug", "-e"],
    ["the airport", "der Flughafen", "-häfen"],
    ["the train station", "der Bahnhof", "-höfe"],
    ["the river", "der Fluss", "-üsse"],
    ["the harbor", "der Hafen", "-äfen"],
    ["the hotel", "das Hotel", "-s"],
    ["the church", "die Kirche", "-n"],
    ["the market", "der Markt", "-Märkte"],
    ["the town hall", "das Rathaus", "-äuser"],
    ["piece", "das Stück", ""],
    ["profession", "der Beruf", "-e"]
    ];
    //*/
    
    //---------------------------------------------------------